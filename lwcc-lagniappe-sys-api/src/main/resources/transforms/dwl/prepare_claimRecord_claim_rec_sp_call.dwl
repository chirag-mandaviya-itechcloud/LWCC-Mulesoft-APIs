...
[
    ...
    payload.'injured-worker'.FirstName, 
    null,   // payload.claimant_initial  new field claimant initial 
    payload.'injured-worker'.LastName, 
    payload.'injured-worker'.vlocity_ins__SocialSecurityNumber__c, 
    ...
    payload.'injured-worker'.MailingStreet,
    payload.'injured-worker'.DT_Care_Of__c,   // payload."claimant_addr2-TBD",
    payload.'injured-worker'.MailingCity,
    dateformat(payload.'injured-worker'.Birthdate),
    dateformat(payload.'injured-worker'.DT_Deceased_Date__c),
    payload.'injured-worker'.Phone,
    mapGender(payload.'injured-worker'.FinServ__Gender__c),
    payload.'injured-worker'.mailingstatecode,
	substring(payload.'injured-worker'.MailingPostalCode, 0, 5),
    if(!isEmpty(substring(payload.'injured-worker'.MailingPostalCode, 5, 10))) (
        substring(payload.'injured-worker'.MailingPostalCode, 5, 10)) else null, // payload."claimant_zip4",
    ...
    null, // payload."eye_read_code1-TBD",
    ...
    null, //payload.'claim-participant'.DT_Marital_Status__c,
    ...
    payload.'claim-object'.DT_Claim_Completed_By__c,
    ...
    null, //if(!isEmpty(payload.'claim-participant'.DT_Dependent_Children__c)) payload.'claim-participant'.DT_Dependent_Children__c as Number else null,
    null,   // payload."num_oth_dependents-TBD",
    null, //mapBool(payload.'claim-participant'.DT_Salary_Continued__c),
    null, //dateformat(payload.'claim-participant'.DT_Last_Date_Worked__c), 
    ...
    if(payload.'claim-object'.DT_Country__c == 'United States') "US" else "XX", // Mapping country only for US 
    null,   // payload."prior_claimant_country", 
    null, //mapBool(payload.'claim-participant'.DT_At_same_wage__c),  
    null, //payload.'claim-participant'.DT_Race__c,
    null,   // payload."prior_claimant_county",   
    null, //payload.'claim-participant'.DT_County__c,
    payload.'claim-object'.DT_Reporting_Mechanism__c,
    null, // payload.'claim-object'.Manual_Owner_Assigned_Alias__c (not what it's called in the oracle procedure)
    ...
    mapBool(payload.'injured-worker'.DT_Date_of_Birth_Not_Available__c),   // payload."claimant_dob_not_available",
    ...
    null, //mapBool(payload.'claim-participant'.DT_Physical_restriction__c),    
    null,   // payload."released_rtw_date",
    null, //payload.'claim-participant'.DT_Days_Worked_Per_Week__c, 
    ...
    payload.'injured-worker'.DT_Medicare_Eligibility__c,
